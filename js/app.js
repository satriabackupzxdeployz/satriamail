    ...
    <div id="homeTab" class="tab-view active">
      <div class="profile-card" id="profileAvatar">
        <div class="profile-card-avatar">
          <img src="${currentUser.avatar || `https://ui-avatars.com/api/?name=${currentUser.name}&background=111&color=fff`}" alt="">
        </div>
      ...
      <div class="modal-content">
        ...
        <div class="profile-settings">
          <div class="settings-avatar">
            <img src="${currentUser.avatar || `https://ui-avatars.com/api/?name=${currentUser.name}&background=111&color=fff`}" id="settingsAvatarImg">
          </div>
  ...
  try {
    ...
    document.getElementById('modalReplyBtn')?.addEventListener('click', () => {
      if (replyToMessage) {
        ...
        if (targetName) {
          targetName.value = replyToMessage.from.split('@')[0];
        }
        document.getElementById('messageModal').classList.remove('show');
    ...
    window.viewUserProfile = async function(email) {
      try {
        ...
        for (const [id, data] of Object.entries(snapshot.val() || {})) {
          const userEmail = decryptData(data.email);
          if (userEmail === email) {
            document.getElementById('viewAvatarImg').src = decryptData(data.avatar) || `https://ui-avatars.com/api/?name=${decryptData(data.name)}&background=111&color=fff`;
            document.getElementById('viewName').value = decryptData(data.name);
            document.getElementById('viewEmail').value = userEmail;
            document.getElementById('viewBio').value = decryptData(data.bio) || '';
    ...
    console.error('Attach home events error:', error);
  }
}
...
async function loadPublicChats() {
  try {
    ...
    console.error('Load public chats error:', error);
  }
}
