...
import { 
  getDatabase, ref, set, push, get, query, orderByChild, equalTo, update, remove 
...
// Simple XOR encryption (lebih ringan)
...
// Fungsi untuk cek akses sederhana
async function verifyAccess() {
  try {
    const accessRef = ref(database, '_access');
    const snapshot = await get(accessRef);
    if (!snapshot.exists()) {
      await set(accessRef, { key: encryptData(ACCESS_KEY), initialized: true });
      return true;
    }
    const data = snapshot.val();
    const decryptedKey = decryptData(data.key);
    return decryptedKey === ACCESS_KEY;
  } catch {
    return false;
  }
}
